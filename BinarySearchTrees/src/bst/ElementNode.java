 ...
 */
public class ElementNode<T extends Comparable<T>> implements BinarySearchNode<T> {
  ...
  @Override
  public BinarySearchNode<T> add(T data) {
  ...
  @Override
  public int size() {
    int left = leftNode.size() + 1;
    int right = rightNode.size() + 1;
    return (left + right) - 1;
  }

  @Override
  public int height() {
    int left = leftNode.height() + 1;
    int right = rightNode.height() + 1;
  ...
  public String preOrder() {
    ...
    if (left.length() > 0) {
      objectstring = objectstring + " " + left;
    }
    String right = this.rightNode.preOrder();
    if (right.length() > 0) {
      return objectstring + " " + right;
    }
  ...
  public String inOrder() {
    ...
    objectstring = objectstring + this.object.toString();
  ...
  public String postOrder() {
    ...
    if (right.length() > 0) {
      objectstring = objectstring + right + " ";
    }
